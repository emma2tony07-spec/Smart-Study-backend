...
#  4. Start command:  gunicorn app_render:app
...
ALLOWED_ORIGINS = [
    "http://localhost:8080",
    "http://localhost:5500",
    "https://your-studyos-frontend.netlify.app",  # update this
...
FROM_NAME             = "StudyOS"
MASTER_OPENROUTER_KEY = os.environ.get("MASTER_OPENROUTER_KEY", "")
...
DEFAULT_CONFIG = {
    ...
    "summarize": "meta-llama/llama-3.1-8b-instruct:free",
    "grade":     "mistralai/mistral-7b-instruct:free",
    "simplify":  "google/gemma-2-9b-it:free",
    "visualize": "meta-llama/llama-3.1-8b-instruct:free",
}
...
#  (same as Termux version — identical code block)
# ═══════════════════════════════════════════════════════════════
pending_reminders = {}
...
# ═══════════════════════════════════════════════════════════════
#  AI FUNCTIONS (identical to Termux)
# ═══════════════════════════════════════════════════════════════
...
def get_usage():
    ...
    return jsonify({
        "uid": uid, "used": count, "limit": DAILY_FREE_LIMIT,
        "remaining": max(0, DAILY_FREE_LIMIT - count),
        "limited": limited, "has_own_key": bool(personal_key), "resets": "midnight"
    })

@app.route('/models', methods=['GET'])
def get_models():
    return jsonify({"available": AVAILABLE_MODELS, "config": model_config,
...
def set_model_config():
    ...
    for feature in ["analyze","summarize","grade","simplify","visualize"]:
        if feature in data: model_config[feature] = data[feature]
...
def validate_key():
    ...
    if not key: return jsonify({"valid": False, "error": "No key provided"}), 400
    try:
        headers = {"Authorization": f"Bearer {key}", "Content-Type": "application/json"}
        resp    = requests.get(f"{OPENROUTER_BASE_URL}/models", headers=headers, timeout=10)
        if resp.status_code == 200:   return jsonify({"valid": True})
        elif resp.status_code == 401: return jsonify({"valid": False, "error": "Invalid API key."})
        else: return jsonify({"valid": False, "error": f"OpenRouter returned {resp.status_code}."})
    except Exception as e:
...
def analyze_file():
    ...
    try:
        ...
        print(f"📄 {len(text)} chars | model: {model_config['analyze']} | uid: {uid}")
...
def summarize():
    ...
    try:
        ...
        if not text.strip(): return jsonify({"error": "Empty chapter content"}), 400
        result = ai_summarize_and_question(text, api_key)
        return jsonify({"summary": result.get("summary","No summary."), "questions": result.get("questions",[])})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def grade():
    ...
    try:
        ...
        if not student_answer: return jsonify({"error": "No answer provided"}), 400
...
def simplify():
    ...
    try:
        ...
        text = file.read().decode('utf-8', errors='ignore')
        if not text.strip(): return jsonify({"error": "Empty chapter content"}), 400
        return jsonify(ai_simplify(text, api_key))
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def visualize_data():
    ...
    try:
        ...
        text = file.read().decode('utf-8', errors='ignore')
        if not text.strip(): return jsonify({"error": "Empty chapter content"}), 400
        return jsonify(ai_visualize(text, api_key))
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def send_confirmation():
    try:
        ...
        task_dt  = datetime.fromisoformat(f"{task['date']}T{task['time']}")
        time_str = task_dt.strftime("%-I:%M %p")
        send_email(to_address=user_email, to_name=user_name,
                   subject=f"✅ Task Scheduled: {task['title']} at {time_str}",
                   task=task, course_name=course_name,
        ...
        with reminder_lock:
            pending_reminders[task_id] = {
                ...
                "user_name": user_name, "course_name": course_name,
                "reminder_sent": False
            }
...
def health():
    return jsonify({
        "status": "healthy", "provider": "openrouter",
        "model_config": model_config, "daily_limit": DAILY_FREE_LIMIT,
        "master_key_set": bool(MASTER_OPENROUTER_KEY)
    })

# Render needs this exposed as `app` for gunicorn
# gunicorn app_render:app